...
function logout() {
  if (confirm("Are you sure you want to logout?")) {
    ["isLoggedIn", "userEmail", "userName", "userId"].forEach((k) =>
  ...
  try {
    ...
    <div>Target Price: $${
      product.target_price != null
        ? Number(product.target_price).toFixed(2)
        : "—"
    }</div>
`;
    ...
    products.forEach((product) => {
      ...
      if (!isNaN(target) && !isNaN(current) && current <= target) {
        alertTextEl.textContent = `🎉 Price dropped for ${
...
// New Remove Product feature 
// --- Remove product ---
async function removeProduct(productId) {
    const confirmDelete = confirm("Remove this item from tracking?");
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (
          targetPrice !== null &&
          data.current_price !== null &&
          data.current_price <= targetPrice
        ) {
          document.getElementById(
            "alertText"
          ).textContent = `🎉 Price dropped for ${itemName}: ${priceText}`;
...
async function updateAllPrices() {
  try {
    const response = await fetch(
      `/sprint2/api/update-prices?user_id=${userId}`
    );
    const data = await response.json();
...
setInterval(() => {
  if (document.getElementById("autoUpdateToggle").checked) updateAllPrices();
}, 24 * 60 * 60 * 1000);
